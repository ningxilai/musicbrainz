...
- 20+ 种实体：search / lookup / browse / relations
- 完整关系支持：解析和查询实体间关系
...
M-x musicbrainz-org-insert-relations
M-x musicbrainz-org-insert-disc-id-releases
M-x musicbrainz-org-insert-isrc-recordings
M-x musicbrainz-org-insert-iswc-works
M-x musicbrainz-org-insert-collection-releases
...
;; 关系查询
(musicbrainz-lookup-artist-relations "artist-mbid")
(musicbrainz-lookup-release-relations "release-mbid")
(musicbrainz-lookup-recording-relations "recording-mbid")
(musicbrainz-lookup-work-relations "work-mbid")
(musicbrainz-lookup-label-relations "label-mbid")

;; Disc ID / ISRC / ISWC / PUID
(musicbrainz-lookup-disc-id "disc-id")
(musicbrainz-lookup-disc-id-releases "disc-id")
(musicbrainz-lookup-isrc "isrc-code")
(musicbrainz-lookup-isrc-recordings "isrc-code")
(musicbrainz-lookup-iswc "iswc-code")
(musicbrainz-lookup-iswc-works "iswc-code")
(musicbrainz-lookup-puid "puid-code")

;; Collection
(musicbrainz-lookup-collection "collection-id")
(musicbrainz-lookup-collection-releases "collection-id")
(musicbrainz-lookup-collection-artists "collection-id")
(musicbrainz-lookup-collection-recordings "collection-id")

;; CDStub / FreeDB
(musicbrainz-lookup-cdstub "cdstub-id")
(musicbrainz-lookup-freedb "freedb-id")
```
...
| 实体          | 查找函数                              | Org 插入                          |
|---------------|---------------------------------------|----------------------------------|
| Relation      | `lookup-*-relations`                  | `insert-relations`               |
| Disc ID       | `lookup-disc-id`, `lookup-disc-id-releases` | `insert-disc-id-releases`   |
| ISRC          | `lookup-isrc`, `lookup-isrc-recordings`     | `insert-isrc-recordings`   |
| ISWC          | `lookup-iswc`, `lookup-iswc-works`         | `insert-iswc-works`        |
| PUID          | `lookup-puid`                         | —                                |
| Collection    | `lookup-collection`, `lookup-collection-*`  | `insert-collection-releases` |
| CDStub        | `lookup-cdstub`                       | —                                |
| FreeDB        | `lookup-freedb`                       | —                                |
...
```elisp
;; 查询艺术家的所有关系
(musicbrainz-lookup-artist-relations "4b585938-f271-45e2-b19a-91c634b5e396")

;; 返回格式：(:artist artist :relations relations :relation-lists relation-lists)

;; 每个关系包含：
;; - type: 关系类型（如 "member of band", "performance"）
;; - target-type: 目标实体类型（如 "artist", "release"）
;; - target-id: 目标 MBID
;; - begin-date, end-date, ended: 时间信息
```
...
(define-key org-mode-map (kbd "C-c m R") #'musicbrainz-org-insert-relations)
(define-key org-mode-map (kbd "C-c m d") #'musicbrainz-org-insert-disc-id-releases)
```
